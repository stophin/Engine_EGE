...
INT EP_TruncateImage(EPTYPE x, EPTYPE y, EPTYPE w, EPTYPE h, EIMAGE image, EPTYPE tx, EPTYPE ty, EPTYPE tw, EPTYPE th, EIMAGE image_bk, INT bk, ECOLOR c, PEPoint cp, PointF * ds, PointF * geom)
{
	int reverse = 0;
	if (w < 0) {
		w = -w;
	...
	int mh = height;
	if (ds) {
		if (!DEF_ISZERO(ds->X)) sx = ds->X;
		if (!DEF_ISZERO(ds->Y)) sy = ds->Y;
	}
	if (geom) {
		if (!DEF_ISZERO(geom->X)) mw = geom->X;
		if (!DEF_ISZERO(geom->Y)) mh = geom->Y;
	}
	if (x > width || y > height)
	{
		return 0;
	...
	for (j = 0; j < h; j++)
	{
		for (i = 0; i < w; i++)
		{
			...
			{
				if (reverse) {
					if (EP_ISZERO(cp->x - ((int)(x + (width - i))) % width) &&
						EP_ISZERO(cp->y - (int)(y + j)))
					{
						isin = true;
					}
				}
